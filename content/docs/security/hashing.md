...
您可以使用 `hash` 服务在应用程序中对用户密码进行哈希处理。AdonisJS 原生支持 `bcrypt`、`scrypt` 和 `argon2` 哈希算法，并且具备 [添加自定义驱动程序](#creating-a-custom-hash-driver) 的能力。
...
您 [无法将哈希值转换回纯文本](https://cheatsheetseries.owasp.org/cheatsheets/Password_Storage_Cheat_Sheet.html#hashing-vs-encryption)，哈希处理是一个单向过程，一旦生成哈希，就无法检索原始值。

然而，哈希处理提供了一种验证给定纯文本值是否与现有哈希匹配的方法，您可以使用 `hash.verify` 方法执行此检查。
...
Argon 是推荐用于对用户密码进行哈希处理的算法。要使用 AdonisJS 哈希服务的 argon，您必须安装 [argon2](https://npmjs.com/argon2) npm 包。

```sh
npm i argon2
```

我们为 argon 驱动程序配置了安全的默认值，但您可以根据应用程序需求调整配置选项。以下是可用选项的列表。
...
要使用 AdonisJS 哈希服务的 Bcrypt，您必须安装 [bcrypt](http://npmjs.com/bcrypt) npm 包。
...
由于您将使用 `hash` 服务对用户密码进行哈希处理，因此可能会发现将逻辑放置在 `beforeSave` 模型钩子中很有帮助。

:::note

如果您使用的是 `@adonisjs/auth` 模块，则无需在模型中哈希密码。`AuthFinder` 会自动处理密码哈希，确保您的用户凭据得到安全处理。有关此过程的更多信息，请[在此处](../authentication/verifying_user_credentials.md#hashing-user-password)了解。
...
如果您的应用程序使用多个哈希驱动程序，可以使用 `hash.use` 方法在它们之间切换。
...
哈希值通常是一个缓慢的过程，会使测试变慢。因此，您可以考虑使用 `hash.fake` 方法模拟哈希服务，以禁用密码哈希。

在以下示例中，我们使用 `UserFactory` 创建了 20 个用户。由于您使用模型钩子来哈希密码，这可能需要 5-7 秒（具体取决于配置）。
...
哈希驱动程序必须实现 [HashDriverContract](https://github.com/adonisjs/hash/blob/main/src/types.ts#L13) 接口。此外，官方哈希驱动程序使用 [PHC 格式](https://github.com/P-H-C/phc-string-format/blob/master/phc-sf-spec.md) 来序列化哈希输出以便存储。您可以查看现有驱动程序的实现，了解它们如何使用 [PHC 格式化程序](https://github.com/adonisjs/hash/blob/main/src/drivers/bcrypt.ts) 来生成和验证哈希。
...
- `PbkdfConfig`：您希望接受的配置的 TypeScript 类型。
