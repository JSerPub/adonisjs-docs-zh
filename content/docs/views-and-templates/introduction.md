...
AdonisJS 非常适合在 Node.js 中创建传统的服务器端渲染应用程序。如果你喜欢使用后端模板引擎来输出 HTML，无需虚拟DOM和构建工具的额外开销，那么本指南非常适合你。
...
AdonisJS 也非常适合创建混合应用程序，这些应用程序在服务器端渲染 HTML，然后在客户端激活你的 JavaScript。这种方法在开发人员中很受欢迎，他们希望使用 `Vue`、`React`、`Svelte`、`Solid` 或其他库来构建交互式用户界面，但仍然需要一个完整的后端堆栈来处理服务器端问题。
